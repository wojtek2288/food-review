...
export const RestaurantDetailsScreen = ({
  ...
  const [restaurant, setRestaurant] = useState<
    RestaurantDetailsResponse | undefined
  >(undefined);
  const detailsReq = {
  ...
  const { run: detailsRun, response: detailsResponse } =
    useRestaurantDetailsQuery(detailsReq);
  const {
    run: reviewsRun,
    response: reviewsResponse,
    isLoading: areReviewsLoading,
  } = useRestaurantReviewsQuery(reviewsReq);
  const {
    run: dishesRun,
    response: dishesResponse,
    isLoading: areDishesLoading,
  } = useRestaurantDishesQuery(dishesReq);
  const {
    run: addReviewRun,
    requestSuccessful,
  ...
  const onEndReached = () => {
    if (selectedIndex === 0) {
      if (
        reviewsCurrentPage * defaultPageSize >= reviewsTotalCount ||
      ...
      if (
        dishesCurrentPage * defaultPageSize >= dishesTotalCount ||
        dishesCurrentPage == 0 ||
        areDishesLoading
      ) {
        return;
      ...
      {restaurant === undefined ? (
        ...
        <View>
          ...
          <FlatList
            ...
            ListFooterComponent={() => (
              <>
                {(areReviewsLoading && selectedIndex === 0) ||
                (areDishesLoading && selectedIndex === 1) ? (
